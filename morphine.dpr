...
begin
 WriteLn;
 WriteLn('Morphine v3.5');
 WriteLn('DLL, EXE Files Packer & Encryptor');
 WriteLn('Main Porgramming : Holy_Father & Ratter/29A (Original Authors of Morphine)');
 WriteLn('Updated, fixed and tested by Silent Shield (Dayvo)');
 WriteLn('Some good ideas (Ultra Halt :-0) : Silent Destroyer');
 WriteLn('and xtra thx to DochtorX for his Assembler Helps');
 WriteLn('--------------------------------------------------------------------------');
 WriteLn('Copyright (c) 2005, Silent Software & Silent Shield');
 WriteLn('--------------------------------------------------------------------------');
 WriteLn('Now is best only this !');
 WriteLn('Newly added (by Silent Shield) Sm@ll Import Table Protection !');
 WriteLn('Download more stuff at ssoft.wz.cz,');
 WriteLn('or mail to me : sshield@pobox.sk');
 WriteLn;
end;
...
function GetFSize(const FileName:String) : Integer;
var FileStr:TFileStream;
begin
  FileStr:=TFileStream.Create(FileName, $0040);
  Result:=FileStr.Size;
  FileStr.Free;
end;
...
begin
 ...
 // PackProcedure
 APackFile(MainDataCyp, MDC, MainSize);
 MoveMemory(MainDataCyp, MDC, PackedSize);
 // EndOfPackProcedure

 LSmashNum:=EncoderProc(MainDataCyp); // Encode MainDataCyp
...
begin
 ...
 begin
  ...
  MainSize:=MainRealSize+Cardinal(Random(250)+10);
  MainData:=MyAlloc(MainSize);
  MDC:=MyAlloc(MainSize);
  MainDataCyp:=MyAlloc(MainSize);
  PackedSize:=MainRealSize;
  if (MainData<>nil) and (MainDataCyp<>nil) and (MDC<>nil) then
  begin
   ...
   begin
    ...
    CopyMemory(MDC, MainData, MainSize);
